...
async function onSave(img: GenResultImage, _index: number) {
  await gallery.save({ adapterId: settings.activeProfile!.adapterId, mode: 'text-to-image', prompt: prompt.value, params: params.value, image: img, apiConfig: { endpoint: settings.activeProfile!.config.endpoint, model: settings.activeProfile!.config.model } })
  emit('saved')
}
</script>
